...
export function RolePermissionAssignment({ roleId, roleName }: RolePermissionAssignmentProps) {
    const router = useRouter();
    const { data: permissionsData, isLoading: isLoadingPermissions } = useGetPermissions();
    const { data: rolePermissionsData, isLoading: isLoadingRolePermissions } = useGetRolePermissions(roleId);
    const { mutate: assignPermissions, isPending: isAssigning } = useAssignPermissionsToRole();
    ...
    useEffect(() => {
        if (rolePermissionsData?.data) {
            const permissionIds = rolePermissionsData.data.map(p => p.id);
    ...
    const handleSavePermissions = () => {
        assignPermissions({ roleId, permissionIds: selectedPermissions });
    };
        ...
        <Card className="max-w-4xl mx-auto">
            <CardHeader className="space-y-4">
                <CardTitle className="text-2xl">
                    Phân quyền cho vai trò {roleName ? `"${roleName}"` : ""}
                </CardTitle>
