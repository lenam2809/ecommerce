...
{
    public class GetSystemNotificationsQueryHandler : IRequestHandler<GetSystemNotificationsQuery, Result<PaginatedList<NotificationDto>>>
    {
        ...
        public async Task<Result<PaginatedList<NotificationDto>>> Handle(GetSystemNotificationsQuery request, CancellationToken cancellationToken)
        {
            try
            {
                // Xây dựng biểu thức filter cho system notifications
                Expression<Func<Notification, bool>> filter = notification =>
                    //(notification.Type == ENotificationType.System ||
                    // notification.Type == ENotificationType.Promotion ||
                    // notification.Type == ENotificationType.Maintenance) &&
                    (request.IncludeExpired || notification.ExpiresAt == null || notification.ExpiresAt > DateTime.UtcNow);
                ...
                Func<IQueryable<Notification>, IOrderedQueryable<Notification>> orderBy = query =>
                {
                    return request.SortBy.ToLower() switch
                    {
                        ...
                        "expiresat" => request.IsDescending
                            ? query.OrderByDescending(n => n.ExpiresAt)
                            : query.OrderBy(n => n.ExpiresAt),
            ...
            {
                await _logger.LogExceptionAsync(ex, "GetSystemNotificationsQueryHandler.Handle");
